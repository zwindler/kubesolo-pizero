...
2. Boot, copy this repository to the Pi, then fetch everything the demo needs (network required, ~210 MB):
   ...
   Files land in `/opt/kubesolo-demo`: KubeSolo offline build (all images embedded) and installer, both SHA-256 checked, and the `iptables`/`dropbear` packages. After this, only the nginx and DOOM images (6 MB and 2.4 MB) need the network.
...
| 4 | `./demo/10-os-tuning.sh memory` | `min_free_kbytes`, zram tuning, journal in RAM | seconds |
...
- KubeSolo wipes its images at every start (upstream [#197](https://github.com/portainer/kubesolo/issues/197)): after a reboot, nginx and DOOM are pulled again (a few seconds each).
